    ...
    : Detector(std::move(name))
{
    ...
    SetLength(nose_length + cone_length + zyl_length);
    ...
    switch (type)
    {
        ...
        default:
            G4Exception("BGO::BGO()",
                        "Unknown nose type",
                        FatalException,
                        ("BGO " + GetName() + " has an unknown type.").c_str());
            break;
    }

    SetWidth(nose_front_outer_diameter);
    ...
    auto* mother_zyl_s = new G4Tubs("BGO_" + GetName() + "_mother_zyl_solid",
                                    zyl_inner_diameter / 2.,
                                    zyl_outer_diameter / 2.,
                                    zyl_length / 2.,
                                    0. * deg,
                                    360. * deg);
    auto* mother_cons_s = new G4Cons("BGO_" + GetName() + "_mother_cons_solid",
                                     nose_front_inner_diameter / 2.,
                                     nose_front_outer_diameter / 2.,
                                     cone_back_inner_diameter / 2.,
                                     cone_back_outer_diameter / 2.,
                                     (nose_length + cone_length) / 2.,
                                     0. * deg,
                                     360. * deg);
    auto* mother_s = new G4UnionSolid("BGO_" + GetName() + "_mother_solid",
                                      mother_zyl_s,
                                      mother_cons_s,
                                      nullptr,
                                      G4ThreeVector(0., 0., -GetLength() / 2.));
    auto* mother_mat = G4Material::GetMaterial("Galactic");

    SetLogical(new G4LogicalVolume(mother_s, mother_mat, "BGO_" + GetName() + "_mother_lv"));
    auto vis = G4VisAttributes(G4Color(0.1, 0.1, 0.1, 0.1));
    vis.SetForceSolid(true);
    GetLogical()->SetVisAttributes(vis);

    // Lead nose
    auto* nose_s = new G4Cons("BGO_" + GetName() + "_nose_solid",
                              nose_front_inner_diameter / 2.,
                              nose_front_outer_diameter / 2.,
                              nose_back_inner_diameter / 2.,
                              nose_back_outer_diameter / 2.,
                              nose_length / 2,
                              0. * deg,
                              360. * deg);
    ...
    auto* nose_lv = new G4LogicalVolume(nose_s, nose_mat, "BGO_" + GetName() + "_nose_lv");
    nose_lv->SetVisAttributes(nose_vis);
    new G4PVPlacement(nullptr,
                      G4ThreeVector(0., 0., -(GetLength() / 2. + cone_length / 2.)),
                      nose_lv,
                      "BGO_" + GetName() + "_nose",
                      GetLogical(),
                      false,
    ...
    // BGO-Crystal
    auto* bgo_zyl_s = new G4Tubs("BGO_" + GetName() + "_bgo_zyl_solid",
                                 zyl_inner_diameter / 2.,
                                 zyl_outer_diameter / 2.,
                                 zyl_length / 2.,
                                 0. * deg,
                                 360. * deg);
    auto* bgo_cons_s = new G4Cons("BGO_" + GetName() + "_bgo_cons_solid",
                                  cone_front_inner_diameter / 2.,
                                  cone_front_outer_diameter / 2.,
                                  cone_back_inner_diameter / 2.,
                                  cone_back_outer_diameter / 2.,
                                  cone_length / 2.,
                                  0. * deg,
                                  360. * deg);
    auto* bgo_s = new G4UnionSolid("BGO_" + GetName() + "_bgo_solid",
                                   bgo_zyl_s,
                                   bgo_cons_s,
                                   nullptr,
                                   G4ThreeVector(0., 0., -(zyl_length + cone_length) / 2.));
    auto* bgo_mat = G4Material::GetMaterial("BGO");
    auto* bgo_vis = new G4VisAttributes(G4Color(0.8, 0., 0.1));
    bgo_vis->SetForceSolid(true);

    auto* bgo_lv = new G4LogicalVolume(bgo_s, bgo_mat, "BGO_" + GetName() + "_bgo_lv");
    bgo_lv->SetVisAttributes(bgo_vis);
    new G4PVPlacement(
        nullptr, G4ThreeVector(0., 0., 0.), bgo_lv, "BGO_" + GetName() + "_bgo", GetLogical(), false, 0, false);
}
