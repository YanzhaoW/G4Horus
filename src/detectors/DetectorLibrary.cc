...
// TODO: put those constants into a JSON file

// NOLINTBEGIN(cppcoreguidelines-avoid-magic-numbers, readability-magic-numbers)
auto BuildSpecs() -> std::map<std::string, HPGe::_spec>
{
...
auto DetectorLibrary::GetDetector(const std::string& det_id, const std::string& position) -> std::unique_ptr<Detector>
{
    if (det_id == "PIPS")
    {
        return std::make_unique<PIPS>(position);
    }

    // Find spec
    auto det_specs = specifications.find(det_id);
    if (det_specs == specifications.end())
    {
        G4Exception("DetectorLibrary::GetDetector",
                    "Unknown detector",
                    FatalException,
                    ("HPGe " + det_id + " has no specification.").c_str());
    }
    const HPGe::_spec spec = det_specs->second;

    switch (spec.type)
    {
        case HPGe::HPGeCoaxial:
            return std::make_unique<HPGe::Coaxial>(spec, position);
        case HPGe::HPGeHexagonal:
            return std::make_unique<HPGe::Hexagonal>(spec, position);
        case HPGe::HPGeClover:
            return std::make_unique<HPGe::Clover>(spec, position);
        default:
            G4Exception("DetectorLibrary::GetDetector",
                        "Unknown detector type",
                        FatalException,
                        ("HPGe " + det_id + " has an unknown type.").c_str());
            return nullptr;
    }
}
